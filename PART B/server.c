...
int main() {
    ...
    // Wait for initial message from client to confirm connection
    while (1) {
        ...
        if (bytes_received > 0) {
            ...
            printf("Server is ready to send and receive messages\n");
    ...
    while (1) {
        ...
        if (activity > 0) {
            ...
            if (FD_ISSET(STDIN_FILENO, &read_fds)) {
                ...
                send_data_with_retransmission(sockfd, &client_addr, addr_len, buffer, text_len);
                printf("Enter message to send: ");
                fflush(stdout);
            }
        }
